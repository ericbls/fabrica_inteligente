...
void FanucAdapter::getMacros() {
    ...
    for (int i = 0; i < mMacroSampleCount; i++) {
        ...
        if (ret == EW_OK) {
            ...
            gLogger->error("Could not retrieve Macro data at %d for %d: %d", mMacroSample[i],
                           mMacroSample[i]->getNumber(), ret);
        }
    }
}
...
void FanucAdapter::getCritical() {
    ...
    for (int i = 0; i < mCriticalCount; i++) {
        ...
        if (ret == EW_OK) {
            ...
            if (i < num && ((odbalmmsg[i].type == 1) || (odbalmmsg[i].type == 2) || (odbalmmsg[i].type == 5) ||
                            (odbalmmsg[i].type == 6) || (odbalmmsg[i].type == 9) || (odbalmmsg[i].type == 10) ||
                            (odbalmmsg[i].type == 13) || (odbalmmsg[i].type == 19))) {
